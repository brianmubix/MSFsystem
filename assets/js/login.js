...
//LOGIN DETAILS
    ...
    $("#regdetails").on('submit', (function (e) {
        e.preventDefault();
        $("#error").html('');
        ...
        $.ajax({
            url: baseurl + "Login/Registernew", // Url to which the request is send
            type: "POST", // Type of request to be send, called as method
            data: new FormData(this), // Data sent to server, a set of key/value pairs (i.e. form fields and values)
            contentType: false, // The content type used when sending data to the server.
            cache: false, // To unable request pages to be cached
            ...
            success: function (data)   // A function to be called if request succeeds.
            {
                ...
                if (obj.success == "1") {
                    $("#error").html(''); //clearing error message
                    $("#regdetails")[0].reset();
                    $("#regdetails").html("<div  class='alert alert-success' ><center><i class='fa fa-check fa-2x text-success mb50 animated zoomIn'></i><br />" + obj.message + "<br/> Please login <a href='"+baseurl +"Login?u=Member'>HERE</a></center></div>");
